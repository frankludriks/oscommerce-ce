    ...
    protected function _buildBranch($parent_id, $level = 0) {
      ...
      if ( isset($this->_data[$parent_id]) ) {
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
          ...
          if ( $level === 0 ) {
            $result .= '<!-- root start -->' .  PHP_EOL . $this->root_start_string;
          }

          if ( isset($this->_data[$category_id]) ) {
            $result .= '<!-- parent start -->' .  PHP_EOL . $this->parent_start_string;
          }

          $result .= '<!-- child start -->' .  PHP_EOL . $this->child_start_string;
		  ...
		  $result .= $this->child_end_string . PHP_EOL . '<!-- child end -->';

          if ( isset($this->_data[$category_id]) ) {
            $result .= $this->parent_end_string . PHP_EOL . '<!-- parent end -->';;
          }

          if ( $level === 0 ) {
            $result .= $this->root_end_string . PHP_EOL . '<!-- root end -->';;
          }          
        }
      }
